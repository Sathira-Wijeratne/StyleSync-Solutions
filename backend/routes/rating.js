...
router.route("/add").post((req,res)=>{
    ...
    const newRate = new Rate({
        title_orig,customerEmail,noOfRate
    })
...
router.route("/get/:title_orig").get(async(req,res)=>{
    ...
    await Rate.find({"ItemName": title_orig}).then((rate)=>{
        res.json(rate);
    }).catch((err)=>{
        console.log(err);
...
router.route("/get/:customerEmail/:title_orig").get(async(req,res)=>{
    const title_orig = req.body.title_orig;
    const customerEmail = req.body.customerEmail;
    
    await Rate.find({"buyerEmail": customerEmail, "ItemName": title_orig}).then((rate)=>{
        res.json(rate);
    }).catch((err)=>{
        console.log(err);
        res.status(500).send({status:"Opps! Error in loading the rates"});
...
router.route("/update").put(async(req, res)=>{
    ...
    await Rate.findOneAndUpdate({"buyerEmail" : customerEmail, "itemName": title_orig}, updateRate).then(()=>{
